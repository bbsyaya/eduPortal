...
app.controller('gpsController', function ($rootScope, $scope, $http,$location) {
    ...
    if (localStorage.getItem("login") == 'true') {
        ...
        $("#loading").show();
        $http.get('http://58.96.190.110:8021/f/edu/gps?uid='+$scope.user.id).
    ...
    $scope.relocateGps = function(){
        ...
        //var url = 'http://58.96.190.110:8021/f/edu/gps/locate?uid='+$scope.user.id+"&address="+encodeURIComponent($scope.gpsNow);
        var postData = {uid:$scope.user.id,address:$scope.gpsNow};
        var config = {};
        $http.post('http://58.96.190.110:8021/f/edu/gps/locate', postData, config
        ).success(function(data, status, headers, config) {
                //成功之后做一些事情
                $scope.gpsNow = data;

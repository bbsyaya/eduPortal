...
app.controller('gpsController', function ($rootScope, $scope, $http,$location) {
    ...
    $scope.relocateGps = function(){
        ...
        //var url = 'http://182.92.129.8:8025/f/edu/gps/locate?uid='+$scope.user.id+"&address="+encodeURIComponent($scope.gpsNow);
        var postData = {uid:$scope.user.id,address:$scope.gpsNow};
        var config = {};
        $http.post('http://182.92.129.8:8025/f/edu/gps/locate', postData, config
        ).success(function(data, status, headers, config) {
                //成功之后做一些事情
                $scope.gpsNow = data;
